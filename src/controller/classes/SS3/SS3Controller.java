 ...
 * @author GeoCodec
 */
public class SS3Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
            ...
            tfSearch.textProperty ().addListener ((observable, oldValue, newValue) -> {
                filteredData.setPredicate (student -> {
                    ...
                    if (student.getName ().toLowerCase ().indexOf (lowerCaseFilter) != -1) {
                        return true;
                    } else if (student.getClasses ().toLowerCase ().indexOf (lowerCaseFilter) != -1) {
                        return true;
                    } else if (student.getDepartment ().toLowerCase ().indexOf (lowerCaseFilter) != -1) {
                        return true;
                    } else if (student.getGender ().toLowerCase ().indexOf (lowerCaseFilter) != -1) {
                        return true;
                    } else if (student.getYear ().toLowerCase ().indexOf (lowerCaseFilter) != -1) {
                        return true;
                    } else if (Integer.valueOf (student.getId ()).intValue () != -1)
                        return true;
                    else
                        return false;
        ...
        try {
            ...
            ResultSet resultSet = con.createStatement().executeQuery("SELECT * FROM students WHERE classes = 'SS3'");
    ...
    private void btnSubject(ActionEvent event) throws IOException {
        Parent home = FXMLLoader.load(getClass().getResource("/View/classes/SS3/enterSubject.fxml"));
        Node node = (Node) event.getSource();
        Stage stage = (Stage) node.getScene().getWindow();
        stage.setScene(new Scene(home));
    }
    ...
    private void btnTimeTable(ActionEvent event) throws IOException {
        Parent home = FXMLLoader.load(getClass().getResource("/View/classes/SS3/enterTimeTable.fxml"));
        Node node = (Node) event.getSource();
        Stage stage = (Stage) node.getScene().getWindow();
        stage.setScene(new Scene(home));
    }
    ...
    private void btnScience(ActionEvent event) throws SQLException {
        ...
        String sql = "select * from students WHERE department = 'Science' AND classes = 'SS3'";
        ResultSet rs = null;
        try {
    ...
    private void btnCommercial (ActionEvent event) throws SQLException {
        ...
        String sql = "select * from students WHERE department = 'Commercial' AND classes = 'SS3'";
        ResultSet rs = null;
        try {
            rs = conn.createStatement().executeQuery(sql);
        } catch (SQLException e) {
    ...
    private void btnArt (ActionEvent event){
        ...
        String sql = "select * from students WHERE department = 'Art' AND classes = 'SS3'";
        ResultSet rs = null;
        try {
            rs = conn.createStatement().executeQuery(sql);
        } catch (SQLException e) {
    ...
    private void btnBroadSheet(ActionEvent event)  throws IOException {
        Parent home = FXMLLoader.load(getClass().getResource("/View/classes/SS3/enterBroadSheet.fxml"));
        Node node = (Node) event.getSource();
        Stage stage = (Stage) node.getScene().getWindow();
        stage.setScene(new Scene(home));
    }
